define(['jquery','biligram/parse', 'biligram/ui', 'biligram/copy', 'bootstrap'], function($, parser, ui, copy) {
    //the parser has failed to create an infant
    //render the errors

    function renderErrors(errors) {
            ui.renderErrors(errors,"#errors");
            $('body > :not(#errors)').hide(); //hide all nodes directly under the body
            $('#errors').appendTo('body').show();
    }

    // render the ui including the table and the graph
    // assumes a good "infant"
    ...
    /**
     * setup the table so it only shows the first row
    ...
    function setupTableTogglers(count) {
        ...
        //hide all rows but the first one
    ...
    /**
     * setup right clicking on the chart popping
