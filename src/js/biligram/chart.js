...
define(['jquery','biligram/calculator', 'highcharts-core', 'highcharts-more'], function(jquery,calculator, Highcharts){
    ...
    const TREATMENTZONEPROPS = {
        ...
        "phototherapy": {
            "High": {
                name: "Phototherapy threshold<br>\u22ef low risk infants, \u2265 38w \u0026 well<br>" +
...
return {
    ...
    /**
     * Main entry point to render the chart. Given an infant, render the
     * bilis in the container provided
     * @param infant - Biligram.Infant
     * @param container - dom ID
     * @param getTooltipText - function, given a bili, return the text to show in the tooltip
     */
    render : function(infant,container,enableTooltips,getTooltipText,getRangeName) {
                ...
                for (var zone in TREATMENTZONEPROPS[zoneType]) {
                    ...
                    series.push($.extend({},props,{
                        ...
                        data: addXCoordinates(
                            TreatmentZone.getSeriesByName(zoneType, zone),
                            TreatmentZone.interval,
                            TreatmentZone.startsAt
                        )
                    }));
                }
            }
        ...
        // Given two arrays of y coordinates, replace it with an array of datapoint objects that includes
        ...
        /**
         * Create the series for plotting from the infant's bili array
         * @param infant
         * @returns {Array}
         */
        function makePoints(infant,getTooltipText) {
            ...
            function convertToPoint(bili) {
                return  {
                    x : bili.hours,
                    y : bili.level,
                    marker : makeMarker(bili) ,
                    dataLabels : makeLabel(bili),
                    customTooltip : getTooltipText(bili)
                };
            }

            function makeLabel(bili) {
                // Usually not visible, but display if tx indicated
                var transfusion = bili.getTreatmentZoneTransfusion() ? "transfusion"+bili.getTreatmentZoneTransfusion() : false;
                var photo = bili.getTreatmentZonePhoto() ? "phototherapy"+bili.getTreatmentZonePhoto() : false;
                var dataLabel;
                if (transfusion || photo || 1) {

                    return {
                        backgroundColor: MARKERCOLORS[transfusion] || MARKERCOLORS[photo],
                        color: (transfusion || photo) ? "white" : "contrast",
                        borderRadius:"3px",
                        align: 'center',
                        style:{"textShadow":"","fontWeight":"regular","textOutline":"1 px 1 px contrast"},
                        x: 0,
                        y: -6,
                        enabled: true,
                        formatter: function () {
            ...
            //make the marker for this bili mapping to correct
